let StationX = function ( NAMESPACE, DEFAULT_LANGUAGE, CURRENT_LANGUAGE, AVAILABLE_LANGUAGES ) {
	...
	let Util = {
		...
		fire: function( event, dataPacket, namespace ){
			Liferay.fire( event, {dataPacket: dataPacket, namespace: namespace} );
		}
	};
	...
	class TermPropertiesForm{
		...
		constructor(){
			...
			this.$termType.off('change').on('change',function(event){
				...
				Util.fire( Events.TERM_PROPERTY_CHANGED, dataPacket, NAMESPACE );
			});
